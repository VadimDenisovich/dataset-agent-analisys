...
function truncateForModel(value: string, maxLength = 5000) {
  if (!value || value.length <= maxLength) return value;
  return `${value.slice(0, maxLength)}\n\n[Вывод обрезан: ${value.length - maxLength} символов скрыто, полный вывод не нужен для итогового отчета]`;
}

function createToolOutputForModel(result: CodeExecutionResult) {
  const chartCount = result.charts?.length ?? 0;
  const modelSummary = [
    result.success === false ? 'Python-код завершился с ошибкой.' : 'Python-код выполнен.',
    chartCount > 0
      ? `Сгенерировано графиков: ${chartCount}. Изображения уже отправлены в интерфейс пользователя, в текст отчета их вставлять не нужно.`
      : 'Графики не сгенерированы.',
    result.output ? `Текстовый вывод:\n${truncateForModel(result.output)}` : '',
    result.error ? `Ошибка:\n${truncateForModel(result.error, 2000)}` : '',
  ]
    .filter(Boolean)
    .join('\n\n');

  return result.error
    ? { type: 'error-text' as const, value: modelSummary }
    : { type: 'text' as const, value: modelSummary };
}
...
export function createExecuteCodeTool(sandbox: Sandbox) {
  return tool({
    ...
    toModelOutput: ({ output }) => createToolOutputForModel(output),
  });
}
